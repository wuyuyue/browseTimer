...
function resourceUrl(url) {
  return url.replace(/http:/ig, '').replace(/https:/ig, '');
};



function jsonReplaceByUrl(url, json) {
  var result = json;
  // if (json.token) {
  //   token = json.token;
  // }
...
function apiUrl(url) {
  ...
  if (url) {
    ...
    if (new RegExp('^http[s]*').test(url)) { // hack for lower version device not support startsWith
      return url;
    }
  }
  ...
  if (filterStatusFlag === true) {
    ...
    if (filterJSONFlag === true) {
      result = result.then(filterJSON)
                     .then(function(json) {
                       return jsonReplaceByUrl(_url, json);
                     });
    }
...
export function putAndPost (url, method, params = {}, headers, credentials = 'include') {
  var defHeader = {
  ...
  return _fetch(fetch(apiUrl(url), {
    method: method,
    headers: defHeader,
    credentials: credentials,
    body: _body
  }), params['timeout'] || config.apiTimeout).then(filterStatus)
                                             .then(filterJSON)
                                             .then(function(json) {
...
export function postFile(url, body, headers, filterStatusFlag = true, filterJSONFlag = true, filterCodeFlag = true, credentials = 'include') {
  var defHeader = {
    'Accept': 'application/json'
  }
